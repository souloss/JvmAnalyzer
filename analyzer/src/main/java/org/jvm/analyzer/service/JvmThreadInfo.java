package org.jvm.analyzer.service;
...
import org.jvm.analyzer.exceptions.ShellException;
import org.jvm.analyzer.model.JavaMemoryHeap;
import org.jvm.analyzer.model.JavaMemoryHisto;
import org.jvm.analyzer.model.JavaStackDump;
import org.jvm.analyzer.util.ShellUtils;
import org.jvm.analyzer.util.StringUtils;
...
public class JvmThreadInfo {
    ...
    private static String getPid(String processName){
        String ret = null;
        switch(ShellUtils.env){
            case "windows":
                ret = ShellUtils.execShellCmd("for /f \"tokens=1\" %i in ('jps ^| findstr \""+ processName +"\"') do @echo %i").trim();
                break;
            case "linux":
                ret = ShellUtils.execShellCmd(javaToolDir.resolve("jps") + " | grep "+ processName +" | awk '{print $1}'");
                break;
        }
    ...
    private static String getThreadStackDump(String processName) {
        return ShellUtils.execShellCmd("\"" + javaToolDir.resolve("jstack") + "\" " + getPid(processName));
    }

    private static String getThreadMemoryMapHeap(String processName) {
        return ShellUtils.execShellCmd("\"" + javaToolDir.resolve("jmap") + "\" -heap " + getPid(processName));
    }

    private static String getThreadMemoryMapHisto(String processName) {
        return ShellUtils.execShellCmd("\"" + javaToolDir.resolve("jmap") + "\" -histo " + getPid(processName));
    }
    ...
    public static void main(String[] args) {
        log.info("marker, format, arg");
         System.out.println("-------------------------------------");
    //     System.out.println(getThreadStackDump("org.eclipse.equinox.launcher_1.5.400.v20190515-0925.jar"));
     //    System.out.println("-----end----------");
         System.out.println(getThreadMemoryMapHeap("org.eclipse.equinox.launcher_1.5.400.v20190515-0925.jar"));
        // System.out.println(getThreadMemoryMapHisto("org.eclipse.equinox.launcher_1.5.400.v20190515-0925.jar"));
    }
}
